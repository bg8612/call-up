...
const readStoredRoomSession = (): StoredRoomSession | null => {
  ...
  try {
    ...
    const sessionToken = String(parsed.sessionToken ?? '').trim() || undefined;

    if (!roomId || !displayName) {
      return null;
    }

    return { roomId, displayName, clientSessionId, sessionToken };
  } catch {
    return null;
  }
};
        ...
        <section className="w-full rounded-2xl border border-[var(--border)] p-6 sm:p-8">
          ...
          >
            <label className="grid gap-2">
              ...
              <Input name="displayName" placeholder="Ваше имя" required />
            </label>
            <label className="grid gap-2">
...
function App() {
  ...
  const currentRoomId = normalizeRoomId(prefilledRoomId || readPrefilledRoomId(window.location.href) || '');
  const currentInviteLink = useMemo(
    () => buildInviteLink(window.location.origin + window.location.pathname, currentRoomId),
    [currentRoomId]
  );
  const isInviteCopied = Boolean(copiedInvite && copiedInvite === currentInviteLink);
  ...
  useEffect(() => {
    if (!savedSession || !resolvedSessionToken || joinState !== 'joined') {
      return;
    }

    if (savedSession.sessionToken === resolvedSessionToken) {
      return;
    }
  ...
  }, [joinState, resolvedSessionToken, savedSession]);
              ...
              <div className="grid h-full min-h-0 grid-rows-[auto_minmax(0,1fr)_auto] gap-3 overflow-hidden p-3">
                ...
                <div className="shrink-0 rounded-2xl bg-[var(--surface-muted)] p-4 text-sm">
                  ...
                  <Button
                    variant="secondary"
                    className={cn(
                      'mt-3 w-full transition-colors',
                      isInviteCopied && 'border-[var(--accent-border)] bg-[var(--accent)] text-white hover:bg-[#6e8cff]'
                    )}
